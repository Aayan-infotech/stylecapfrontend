...
const StyleCapsuleManagement = () => {
    ...
    const handleView = async (userId) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`http://54.236.98.193:3555/api/myStyleCapsule/getStyle?userId=${userId}`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            })
    ...
    const fetchStyleData = async (date) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`http://54.236.98.193:3555/api/myStyleCapsule/styleByDate/${date}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                }
            });
                                                ...
                                                {item.picture && item.picture.length > 0 ? (
                                                    item.picture.map((picture, innerIndex) => (
                                                        <CCol xs="12" md="6" className="text-center" key={`${outerIndex}-${innerIndex}`}>
                                                            <img
                                                                src={`http://54.236.98.193:3555/uploads/${picture}`}
