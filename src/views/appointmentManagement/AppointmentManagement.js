...
const AppointmentManagement = () => {
  ...
  const handleView = async (userId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`http://18.209.91.97:3555/api/appointment/get-appointment/${userId}`,
        {
  ...
  const fetchAppointments = async (id) => {
    try {
      setLoading(true);
      const response = await axios.get(`http://18.209.91.97:3555/api/appointment/get-appointment?userId=${id}`);
  ...
  const handleAction = async (userId, id, action) => {
    try {
      await axios.put(`http://18.209.91.97:3555/api/appointment/approve-appointment/${userId}`, {
        appointmentId: id,
  ...
  const handleDelete = async (userId, appointmentId) => {
    try {
      ...
      if (confirmDelete) {
        await axios.delete(`http://18.209.91.97:3555/api/appointment/delete-appointment/${appointmentId}`)
        handleView(userId);
      }
    }
    catch (err) {
