...
const SubscriptionManagement = () => {
  ...
  const changeSubscription = async(id, option) => {
    try{
      const token = localStorage.getItem('token');
      const response = await axios.put(`http://54.236.98.193:3555/api/subscription/update-subscription/${id}`,{option},
        {
          headers: {
            'Authorization': `Bearer ${token}`
        }
  ...
  const subscriptionAnalytics = async() => {
    try{
      const response = await axios.get(`http://54.236.98.193:3555/api/subscription/`);
